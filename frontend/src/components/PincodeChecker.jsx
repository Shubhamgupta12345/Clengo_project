...
export default function PincodeChecker({ onServiceable, compact = false }) {
  ...
  return (
    <div className={compact ? "" : "w-full"} data-testid="pincode-checker">
      <form onSubmit={check} className={`flex items-stretch bg-white ${compact ? "rounded-full" : "rounded-full"} shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-black/5 overflow-hidden`}>
        <div className="pl-5 pr-2 flex items-center text-[#D4A017]">
        ...
        <input
          ...
          className={`flex-1 ${compact ? "py-3 text-sm" : "py-5 text-base"} px-3 outline-none bg-transparent placeholder:text-black/30`}
        ...
        <button
          ...
          className={`m-1.5 ${compact ? "px-5 py-2.5 text-sm" : "px-7 py-3.5 text-sm"} bg-[#D4A017] text-black font-bold rounded-full hover:bg-[#B88A14] transition-colors duration-300 active:scale-95 disabled:opacity-70 flex items-center gap-2`}
        >
